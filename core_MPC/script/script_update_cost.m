...
xCost_minZMPmean_PosToRef=classdef_CostPart(CoP_state_preview.Pu_z_mean,...
    ...
    Step_state_preview.f_step(:,1)+(MPC_inputs.left_support-MPC_inputs.right_support)*MPC_inputs.translate_step_cop_ref(1,1)...
    );
...
% zCosts_minCOM_HeightToRef=classdef_CostPart(Step_state_preview.Px_step(:,end),...
%     [],...
%     Step_state_preview.f_step(:,1),...
%     (MPC_inputs.hcom_ref_max_reduce*0+1)*b_fixed_step...
%     );


% xCosts_minStep_StrenchToRef=classdef_CostPart(A_fixed_step,...
%     [],...
%     0,...
%     b_fixed_step...
%     );
...
xH=MPC_inputs.OptimCostWeight(1)*blkdiag(Cost_minCOM_control.H,zeros(size(Step_state_preview.Pu_step,2)))...
    ...
    +MPC_inputs.OptimCostWeight(3)*0*xCost_minZMPmean_PosToRef.H;
...
xf=MPC_inputs.OptimCostWeight(1)*[Cost_minCOM_control.f;zeros(size(Step_state_preview.Pu_step,2),1)]...
    ...
    +MPC_inputs.OptimCostWeight(3)*0*xCost_minZMPmean_PosToRef.f;
...
% if isempty(xCosts_minStep_StrenchToRef.f)
% else
%     H=H+MPC_inputs.OptimCostWeight(7)*xCosts_minStep_StrenchToRef.H*0; 
%     f=f+MPC_inputs.OptimCostWeight(7)*xCosts_minStep_StrenchToRef.f*0;
% end
% if isempty(xCosts_minStep_StrenchToRef.f)
if isempty(Step_state_preview.Pu_step) 
else
%     conf=MPC_inputs.step_number_pankle_fixed(1,2)/MPC_inputs.step_number_pankle_fixed(1,3);
    % min Step
    [A_fixed_step,b_fixed_step]=pankle_fixed_path(MPC_inputs.nbKnownSteps,MPC_inputs.N,size(Step_state_preview.Pu_step,2),MPC_inputs.step_number_pankle_fixed);

    xCosts_minStep_StrenchToRef=classdef_CostPart(A_fixed_step(1,:),...
    [],...
    0,...
    b_fixed_step(1)...
    );
    yCosts_minStep_StrenchToRef=classdef_CostPart(A_fixed_step(2,:),...
    [],...
    0,...
    b_fixed_step(2)...
    );
    H=H+MPC_inputs.OptimCostWeight(7)*xCosts_minStep_StrenchToRef.H+MPC_inputs.OptimCostWeight(7)*yCosts_minStep_StrenchToRef.H; 
    f=f+MPC_inputs.OptimCostWeight(7)*xCosts_minStep_StrenchToRef.f+MPC_inputs.OptimCostWeight(7)*yCosts_minStep_StrenchToRef.f;
end