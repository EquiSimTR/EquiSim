...
legend('COM','CoP','Location','southeast')
...
plot(MPC_outputs_storage.yc,MPC_outputs_storage.zc,'*b')
h1=plot(yc_discret, zc_discret, 'b');
hold off
legend([h1],{['COM wth spd ' num2str(experiment.vcom_1) 'm.s-1']},'Location','southeast')
...
%         zstep_sampling=experiment.zfloor_ref(1:end-15);
        ...
        xstep_sampling=experiment.Px_step_ref(1:end-14,:)*MPC_outputs_storage.xstep;
        ystep_sampling=experiment.Px_step_ref(1:end-14,:)*MPC_outputs_storage.ystep;
        zstep_sampling=experiment.zfloor_ref(1:end-14);
...
%         ydiff_c_step=MPC_outputs_storage.yc-(ystep_sampling-(sign(ystep_sampling))*experiment.translate_step_polyhedron_type(2));
        xdiff_c_step=MPC_outputs_storage.xc(1:size(xstep_sampling,1),:)-(xstep_sampling+experiment.translate_step_polyhedron_type(1));
        ydiff_c_step=MPC_outputs_storage.yc(1:size(xstep_sampling,1),:)-(ystep_sampling-(sign(ystep_sampling))*experiment.translate_step_polyhedron_type(2));
...
%         zstep_sampling=[experiment.zfloor_ref(1:2);experiment.zfloor_ref(1:end-17)];
        ...
        xstep_sampling=[experiment.Px_step_ref(1:2,:);experiment.Px_step_ref(1:end-16,:)]*MPC_outputs_storage.xstep;
        ystep_sampling=[experiment.Px_step_ref(1:2,:);experiment.Px_step_ref(1:end-16,:)]*MPC_outputs_storage.ystep;
        zstep_sampling=[experiment.zfloor_ref(1:2);experiment.zfloor_ref(1:end-16)];
...
%             'b');
        h1=plot(xc_discret,zc_discret,'b');
...
%         h2=plot(xc_discret(no_begin*experiment.T_start*200+(experiment.vcom_change-no_begin)*experiment.T_b*200+1:end-no_end*20),...
%             zc_discret(no_begin*experiment.T_start*200+(experiment.vcom_change-no_begin)*experiment.T_b*200:end-no_end*20),...
%             'r');
        h2=plot(xc_discret, zc_discret,'r');
...
figure(41)
clf
title('COM acceleration along x-axis')
xlabel('t [s]') % x-axis label
ylabel('x com acceleration [m.s^-1]') % y-axis label
hold on
plot([0;experiment.phase_duration_sampling_cumul(1:size(MPC_outputs_storage.xddc,1)-1)],MPC_outputs_storage.xddc)
% plot([0;experiment.phase_duration_sampling_cumul(1:size(experiment.vcom_ref(:,1),1)-1)],experiment.vcom_ref(:,1))
% temp_vcom=smooth(xvcom_ref,16);
% plot([0;phase_duration_sampling_cumul(1:size(temp_vcom,1)-1)],temp_vcom)
% 
...
hold off
legend('COM acc','Location','southeast')
...
figure(133)
clf
times = experiment.phase_duration_sampling_cumul;
times(any(experiment.phase_type_sampling=='stop',2))=[];
% title('trajectories 3D')
xlabel('Anterposterior axis [m]') % x-axis label
ylabel('Mediolateral axis [m]') % y-axis label
axis equal
% axis image
hold on
h_com=0.780678;
omega=sqrt(experiment.g/h_com);
xcom_x = MPC_outputs_storage.xc(1:size(times,1))+...
         MPC_outputs_storage.xdc(1:size(times,1))/omega;
xcom_y = MPC_outputs_storage.yc(1:size(times,1))+...
         MPC_outputs_storage.ydc(1:size(times,1))/omega;
plot(xcom_x,...
    xcom_y,'-r','LineWidth',2)
legend('XCOM','Location','southeast')
hold on
plot(xcom_x(find(experiment.MoS_sampling==1)+1),...
xcom_y(find(experiment.MoS_sampling==1)+1),...
'.k','MarkerSize',20)

% plot(MPC_outputs_storage.xc(1:size(experiment.vcom_ref,1)-no_end),...
%     MPC_outputs_storage.yc(1:size(experiment.vcom_ref,1)-no_end),'-*k')
% legend('COM','Location','southeast')


plot(xz(1:size(times,1)),yz(1:size(times,1)),'-b','LineWidth',2)
...
MoS_AP=0.09;
MoS_ML=0.05; 
x_points = [-robot.backtoankle, -robot.backtoankle, MoS_AP, MoS_AP];  
y_points = [MoS_ML, robot.ystep_l_0+robot.exttoankle, robot.ystep_l_0+robot.exttoankle, MoS_ML];
color = [0, 1, 0];
...
hold off
% legend('COM','CoP','CoP up','CoP down','Location','southeast')
legend('XCOM','MoS','CoP','Location','southeast')
%% Plot results COM velocity
figure(441) 
clf
ax1 = subplot(3,1,1);
ylabel('CoP B velocity [m/s]') % y-axis label
hold on
plot([0;experiment.phase_duration_sampling_cumul(1:size(MPC_outputs_storage.xddc,1)-1)],xdz,'LineWidth',2)
plot([0;experiment.phase_duration_sampling_cumul(1:size(MPC_outputs_storage.xddc,1)-1)],0.3+0*xdz,'LineWidth',2)
axis([0 experiment.phase_duration_sampling_cumul(size(MPC_outputs_storage.xddc,1)-1) -inf inf])
title('CoP velocities')
legend('CoP B velocity','CoP B velocity limit','Location','southeast')
hold off

ax2 = subplot(2,1,2);
ylabel('CoP U-L velocity [m/s]') % y-axis label
xlabel('time [s]') % x-axis label
hold on
plot([0;experiment.phase_duration_sampling_cumul(1:size(MPC_outputs_storage.xddc,1)-1)],ydz,'LineWidth',2)
plot([0;experiment.phase_duration_sampling_cumul(1:size(MPC_outputs_storage.xddc,1)-1)],1.45+0*ydz,'LineWidth',2)
plot([0;experiment.phase_duration_sampling_cumul(1:25)],-0.42+zeros(26,1),'LineWidth',2)
axis([0 experiment.phase_duration_sampling_cumul(size(MPC_outputs_storage.xddc,1)-1) -inf inf])
legend('CoP U-L velocity','CoP U velocity limit','CoP L velocity limit','Location','southeast')
hold off 

% ax2 = subplot(2,1,3);
% ylabel('vz [m/s]') % y-axis label
% xlabel('t [s]') % x-axis label
% hold on
% plot([0;experiment.phase_duration_sampling_cumul(1:size(MPC_outputs_storage.xddc,1)-1)],sqrt(xdz.^2+ydz.^2),'LineWidth',2)
% axis([0 experiment.phase_duration_sampling_cumul(size(MPC_outputs_storage.xddc,1)-1) -inf inf])
% hold off  
% linkaxes([ax1,ax2],'x')
% legend('Rfoot','Lfoot','Location','southeast')
