    ...
    if experiment.phase_type(i)=='b'||experiment.phase_type(i)=='start'||experiment.phase_type(i)=='stop'
        if i<length(experiment.phase_type)
            ...
            if size(experiment.phase_type,1)==1 
                pstep_3d_phase_r=[pstep_3d_phase_r;pstep_3d(2,:);];
                pstep_3d_phase_l=[pstep_3d_phase_l;pstep_3d(1,:);];
            elseif experiment.phase_type(i-1)=='l'
                pstep_3d_phase_r=[pstep_3d_phase_r;pstep_3d(2,:);];
                pstep_3d_phase_l=[pstep_3d_phase_l;pstep_3d(1,:);];
            else
                pstep_3d_phase_r=[pstep_3d_phase_r;pstep_3d(1,:);];
...
if length(locations)==1
    phase_type_enlarge(locations) = ['b'];
else
...
if length(locations)==1
    phase_duration_enlarge(end)=experiment.phase_duration_start;
else
...
if size(pstep_3d_phase_l,1)==1
    viapoint_l = ones(length(pstep_3d_phase_l)+1,3);
else 
...
figure(311)
clf
title('foot accelerations')

ax1 = subplot(3,1,1);
ylabel('ax [m/s^2]') % y-axis label
hold on
plot(aankle_r(:,1))
plot(aankle_l(:,1))
hold off
