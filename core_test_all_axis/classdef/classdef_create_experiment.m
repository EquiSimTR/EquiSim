classdef classdef_create_experiment<handle
    ...
    methods
        ...
        %%
        function obj=classdef_create_experiment_new(obj,phase_duration_type,...
                nb_foot_step,firstSS,...
                kinematic_limit,robot,polyhedron_position,...
                optimweight_filename,...
                walking_type,k)
            ...
            nb_foot_step = 1;
            %% phase duration
            obj.generate_phase_type_new(nb_foot_step,firstSS);
            
            obj.generate_phase_duration_new(phase_duration_type,k);
            
            obj.generate_phase_sampling_new(k);
...
%             obj.generate_optimCostWeight(optimweight_filename);
            ...
            obj.generate_ref(walking_type,robot,k);
        end
        %%
...
%             phase_duration_type_namefile2=['core_test_all_axis/script/config/phase_duration/' phase_duration_type '.m'];
            if isfile(phase_duration_type_namefile)
                run(phase_duration_type_namefile)
%             elseif isfile(phase_duration_type_namefile2)
%                 run(phase_duration_type_namefile2)
            else
            ...
            obj.phase_duration_RT=phase_duration_RT;
            obj.phase_duration_SPT=phase_duration_SPT;
...
%            obj.preview_windows_duration=phase_duration_r+phase_duration_b*2;
            
%             obj.preview_windows_duration=phase_duration_r+phase_duration_b*2;;
            obj.T_r=phase_duration_r/N_r;
            obj.T_l=phase_duration_l/N_l;
            obj.T_b=phase_duration_b/N_b;
            obj.T_RT=phase_duration_RT/N_RT;
            obj.T_SPT=phase_duration_SPT/N_SPT;
        ...
        function obj=generate_phase_duration_new(obj,phase_duration_type,k)
            %% Phase duration
            ...
            phase_duration_(any(obj.phase_type=='l',2))=obj.phase_duration_l;
            phase_duration_(any(obj.phase_type=='b',2))=obj.phase_duration_b;
            phase_duration_(any(obj.phase_type=='RT',2))=obj.phase_duration_RT;
            phase_duration_(any(obj.phase_type=='SPT',2))=obj.phase_duration_SPT;
%             phase_duration_(any(obj.phase_type=='start',2))=obj.phase_duration_start;
            phase_duration_(any(obj.phase_type=='start',2))=obj.phase_duration_sampling_cumul(k);
            phase_duration_(any(obj.phase_type=='stop',2))=obj.phase_duration_stop;
            ...
            phase_duration_iteration_(any(obj.phase_type=='l',2))=obj.N_l;
            phase_duration_iteration_(any(obj.phase_type=='b',2))=obj.N_b;
            phase_duration_iteration_(any(obj.phase_type=='RT',2))=obj.N_RT;
            phase_duration_iteration_(any(obj.phase_type=='SPT',2))=obj.N_SPT;
%             phase_duration_iteration_(any(obj.phase_type=='start',2))=obj.N_start-1;
            phase_duration_iteration_(any(obj.phase_type=='start',2))=k-1;
            phase_duration_iteration_(any(obj.phase_type=='stop',2))=obj.N_stop;
...
%                 phase_type_(end+1,:)="stop";
        ...
        function obj=generate_phase_type_new(obj,nb_foot_step,firstSS)
            %%
            phase_type_="start";
            phase_type_(end+1,:)="RT";
            phase_type_(end+1,:)="SPT";
            for k=1:nb_foot_step
                if firstSS=='r'
                    if mod(k,2)
                        phase_type_=[phase_type_;'r';'b'];
...
%                 phase_type_(end+1,:)="stop";
        ...
        function obj=generate_phase_sampling(obj)
            ...
            for i=1:length(obj.phase_duration)
                switch(obj.phase_type(i))
                    ...
                    case 'RT'
                        phase_type_sampling_temp=repmat(obj.phase_type(i),obj.N_RT,1);
                    case 'SPT'
                        phase_type_sampling_temp=repmat(obj.phase_type(i),obj.N_SPT,1);
                    otherwise
            ...
            switch(obj.phase_type(end))
                ...
                case 'RT'
                    phase_type_sampling_temp=repmat(obj.phase_type(end),ceil(obj.preview_windows_duration/obj.T_RT),1);
                case 'SPT'
                    phase_type_sampling_temp=repmat(obj.phase_type(end),ceil(obj.preview_windows_duration/obj.T_SPT),1);
                otherwise
                    error(['Unknown phase type of phase_type(i) with i = ' num2str(i)])
            end
            phase_type_sampling_=[phase_type_sampling_;phase_type_sampling_temp];
            ...
            obj.MoS_sampling=zeros(length(obj.phase_type_sampling),1);
            ...
            phase_duration_sampling_(any(obj.phase_type_sampling=='r',2))=obj.T_r;
            ...
            phase_duration_sampling_(any(obj.phase_type_sampling=='RT',2))=obj.T_RT;
            phase_duration_sampling_(any(obj.phase_type_sampling=='SPT',2))=obj.T_SPT;
        ...
        function obj=generate_phase_sampling_new(obj,k)
            %% Phase type sampling
            phase_type_sampling_=[];
            for i=1:length(obj.phase_duration)
                switch(obj.phase_type(i))
                    case 'start'
%                         phase_type_sampling_temp=repmat(obj.phase_type(i),obj.N_start-1,1);
                        phase_type_sampling_temp=obj.phase_type_sampling(1:k-1);
                    case 'stop'
                        phase_type_sampling_temp=repmat(obj.phase_type(i),obj.N_stop,1);
                    case 'r'
                        phase_type_sampling_temp=repmat(obj.phase_type(i),obj.N_r,1);
            ...
            obj.phase_type_sampling=phase_type_sampling_;
%             obj.MoS_sampling=zeros(length(obj.phase_type_sampling),1);
            MoS_sampling_=any(obj.phase_type_sampling=='SPT',2);
            oneIndices = find(MoS_sampling_==1); 
            MoS_sampling_(oneIndices(1:end-1))=0; 
            obj.MoS_sampling=MoS_sampling_; 
            %% Phase sampling duration
            phase_duration_sampling_=zeros(length(obj.phase_type_sampling),1);
            phase_duration_sampling_(any(obj.phase_type_sampling=="start",2))=obj.T_start;
            phase_duration_sampling_(any(obj.phase_type_sampling=="stop",2))=obj.T_stop;
        ...
        function obj=generate_phase_decoupling(obj,firstSS)
            ...
            obj.phase_sampling_length(any(phase_type=='RT',2))=[];
            phase_type(any(phase_type=='RT',2))=[];
            obj.phase_sampling_length(any(phase_type=='SPT',2))=[];
            phase_type(any(phase_type=='SPT',2))=[];
            obj.phase_type=phase_type;
            %%
%             phase_type_decouple_=zeros(length(obj.phase_type_sampling),length(obj.phase_type));
            phase_type_decouple_=zeros(length(obj.phase_type_sampling),length(phase_type));
            ...
            %./
%             if length(obj.phase_sampling_length)==1
%             else
%                 phase_type_decouple_(:,1)=[zeros(obj.phase_sampling_length(1)-1,1);ones(obj.phase_sampling_length(4)-obj.phase_sampling_length(1),1);zeros(length(obj.phase_type_sampling)-obj.phase_sampling_length(4)+1,1)];
%                 for i=4:length(obj.phase_sampling_length)-1
%                     phase_type_decouple_(:,i-2)=[zeros(obj.phase_sampling_length(i)-1,1);ones(obj.phase_sampling_length(i+1)-obj.phase_sampling_length(i),1);zeros(length(obj.phase_type_sampling)-obj.phase_sampling_length(i+1)+1,1)];
%                 end
%             end
            %./
            ...
            if obj.phase_type(end)=='start' 
            elseif obj.phase_type(end)=='stop' ||  obj.phase_type(end)=='b'
            ...
            if size(Px_step_ref_,2)==2
                obj.yaw_sampling=zeros(size(Px_step_ref_,1),1);
            else
                yaw_=0;
                ...
                if obj.phase_type(end)=='stop'
                    yaw_(end+1)=0;
                    if size(obj.phase_type,1)==7 %it was 5 before adding RT SPT
                        yaw_ = yaw_';
                    end
                end
                obj.yaw=yaw_;
        ...
        function obj=generate_ref(obj,varargin)
            walking_type=varargin{1};
            robot=varargin{2};
            if nargin > 3
                k=varargin{3};
            end
            ...
            if nargin > 3
                switch(walking_type)
                    case 4
                    run('core_test_all_axis/script/config/pankle_fixed/pankle_fixed_04.m')
                    case 5
                    run('core_test_all_axis/script/config/pankle_fixed/pankle_fixed_05.m')
                end
            else
                switch(walking_type)
                    case 4
                    run('config/pankle_fixed/pankle_fixed_04.m')
                    case 5
                    run('config/pankle_fixed/pankle_fixed_05.m')
                end
            end
            ...
            switch(walking_type)
                case 1
                    vcom_1_=0.315*0; %m.s^-1
                    vcom_2_=0.315*0; %m.s^-1
                    vcom_1_=0.6*0; %m.s^-1
                    vcom_2_=0.6*0; %m.s^-1
                case 2
                    vcom_1_=0.315; %m.s^-1
            ...
            if nargin > 3
                xvcom_ref_(k+obj.N_RT+1:k+obj.N_RT+obj.N_SPT+obj.N_r)=xvcom_ref_(k+obj.N_RT+1:k+obj.N_RT+obj.N_SPT+obj.N_r)+vcom_1_; %m.s^-1
            else
...
%             
            obj.zeta_up_ref=(hcom_ref_)/obj.g;
            obj.zeta_down_ref=obj.zeta_up_ref;
